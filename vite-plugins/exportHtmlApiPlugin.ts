      ...
      server.middlewares.use(async (req: any, res: any, next: any) => {
        ...
        try {
          ...
          if (pathname === '/api/axure-export-code') {
            ...
            console.log('[Axure Export] 开始构建导出代码:', entry.key);
            const startTime = Date.now();
            try {
              const result = await generateAxureExportCode(projectRoot, entry.key);
              const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
              console.log(`[Axure Export] ✅ 构建完成 (${elapsed}s), 代码大小: ${(result.code.length / 1024).toFixed(1)}KB`);
              res.statusCode = 200;
              res.setHeader('Content-Type', 'text/javascript; charset=utf-8');
              res.end(result.code);
            } catch (buildError: any) {
              const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
              console.error(`[Axure Export] ❌ 构建失败 (${elapsed}s):`, buildError?.message || buildError);
              if (buildError?.stack) console.error('[Axure Export] 堆栈:', buildError.stack);
              return sendJSON(res, 500, { error: `构建失败: ${buildError?.message || '未知错误'}` });
            }
            return;
          }
