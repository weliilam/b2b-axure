...
export async function buildExportIndexBundle(
  ...
  console.log('[Axure Bundle] 开始构建导出代码:', descriptor.key);
  const buildStart = Date.now();
  let axureExport;
  try {
    axureExport = await generateAxureExportCode(projectRoot, descriptor.key);
    const elapsed = ((Date.now() - buildStart) / 1000).toFixed(1);
    console.log(`[Axure Bundle] ✅ 构建完成 (${elapsed}s)`);
  } catch (err: any) {
    const elapsed = ((Date.now() - buildStart) / 1000).toFixed(1);
    console.error(`[Axure Bundle] ❌ 构建失败 (${elapsed}s):`, err?.message || err);
    if (err?.stack) console.error('[Axure Bundle] 堆栈:', err.stack);
    throw err;
  }
