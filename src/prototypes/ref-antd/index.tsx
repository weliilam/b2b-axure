...
import './style.css';
import React, { useState, useCallback, useImperativeHandle, forwardRef, useEffect, useRef, useMemo, Suspense, lazy } from 'react';
...
const DASHBOARD_MENU_ITEMS: DashboardMenuItem[] = [
  { key: 'dashboard', label: '仪表盘', icon: 'dashboard' },
  {
    ...
    children: [
      ...
      { key: 'edit-modal', label: '编辑弹框' },
      { key: 'orders_refund', label: '退款管理' }
    ]
  },
  { key: 'users', label: '用户管理', icon: 'user' },
  { key: 'finance', label: '财务管理', icon: 'shop',
    children: [
      { key: 'tax-list', label: 'B2B税号管理' },
    ]
  },
...
const Component = forwardRef<AxureHandle, AxureProps>(function EcommerceDashboard(innerProps, ref) {
  ...
  const handleMenuSelect = useCallback((key: string) => {
    ...
    // 如果点击的是 order-list 或 edit-modal，直接跳转
    if (key === 'order-list') {
      window.location.href = '/prototypes/order-list';
      return;
    }
    if (key === 'edit-modal') {
      window.location.href = '/prototypes/edit-modal';
      return;
    }
    if (key === 'tax-list') {
      window.location.href = '/prototypes/tax-list';
      return;
    }
  }, []);
